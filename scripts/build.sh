...
process_output_iso() {
  ...
  # Use privilege escalation command to write the checksum file
  if [[ -n "$PRIV_CMD" ]]; then
    if ! $PRIV_CMD bash -c "echo \"$checksum  $iso_name\" > \"$checksum_file_path\""; then
...
copy_package_list() {
  ...
  local package_list_source="${WORK_DIR}/iso/prismlinux/pkglist.x86_64.txt"
  ...
  if [[ -f "$package_list_source" ]]; then
    ...
    cp_command+=("cp" "$package_list_source" "$package_list_dest")
